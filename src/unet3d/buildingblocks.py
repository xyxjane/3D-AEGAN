...
class AR_Encoder_block(nn.Module):
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, padding=1,
        norm_op_kwargs=None, nonlin_kwargs=None, apply_pooling=True):
        super(AR_Encoder_block, self).__init__()
        if nonlin_kwargs is None:
            nonlin_kwargs = {'negative_slope': 0.1, 'inplace': True}
        if norm_op_kwargs is None:
            norm_op_kwargs = {'eps': 1e-3, 'affine': True, 'momentum': 0.1}

        self.nonlin_kwargs = nonlin_kwargs
        self.norm_op_kwargs = norm_op_kwargs

        self.conv = nn.Conv3d(in_channels, out_channels,conv_kernel_size,bias=False, padding=padding)
        self.bn = nn.BatchNorm3d(out_channels,**self.norm_op_kwargs)
...
class AR_Decoder_block(nn.Module):
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, padding=1,
        norm_op_kwargs=None, nonlin_kwargs=None, apply_transconv=True):
        super(AR_Decoder_block, self).__init__()
        if nonlin_kwargs is None:
        ...
        self.conv = nn.Conv3d(in_channels, out_channels,conv_kernel_size,bias=False, padding=padding)
        self.bn = nn.BatchNorm3d(out_channels,**self.norm_op_kwargs)
        self.relu = nn.ReLU(**self.nonlin_kwargs)

        if apply_transconv:
            # self.transconv = True
                self.transconv = nn.ConvTranspose3d(out_channels,out_channels,kernel_size=2, stride=2)
        else:
            self.transconv = None
    ...
    def forward(self, x):
        ...
        out = self.bn(out)
        out = self.relu(out)
        # input_shape = out.size()
        # depth = input_shape[2]
        # width = input_shape[3]
        # height = input_shape[4]
        # new_size = [2*depth,2*width, 2*height]
        if self.transconv is not None:
            out = self.transconv(out)
            # out = F.interpolate(out,size=new_size)
        else:
            pass
        return out
...
class Decoder_nocat(nn.Module):
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
                 conv_layer_order='gcr', num_groups=8, mode='nearest', padding=1, upsample=True):
        super(Decoder_nocat, self).__init__()
