...
from skimage.metrics import adapted_rand_error, peak_signal_noise_ratio, mean_squared_error, normalized_root_mse, structural_similarity
...
class PSNR:
    ...
    def __call__(self, input, target):
        input, target = convert_to_numpy(input, target)
        true_min, true_max = np.min(target), np.max(target)
        data_range = true_max-true_min
        return peak_signal_noise_ratio(target, input,data_range=data_range)
...
class NRMSE:
    """
    Computes NRMSE between input and target
    """
...
class SSIM:
    """
    Computes SSIM between input and target
    """
