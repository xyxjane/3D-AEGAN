...
from unet3d.metrics import SSIM
...
class ContextuaLoss(nn.Module):
    def __init__(self, squeeze_channel=True):
        super(ContextuaLoss, self).__init__()
        self.squeeze_channel = squeeze_channel
        self.divided_points = [0, 32, 64, 96, 128, 160, 192]
    ...
    def forward(self, input, target):
        ...
        if self.squeeze_channel:
            ...
            for i in self.divided_points:
                for j in self.divided_points:
                    patched_input = input[:,:,i:i+32,j:j+32]
                    patched_target = target[:,:,i:i+32,j:j+32]
                    # loss = contextual_loss(patched_input, patched_target)
                    # print(loss)
                    loss = self.contextual_loss(patched_input, patched_target)
                    # print(loss)
                    loss_sum = loss_sum+loss
            loss_sum = loss_sum/16.0
        else:
            loss_sum = contextual_loss(input, target)
        return loss_sum
...
def get_losses_criterion(config):
    """
    Returns the loss function based on provided configuration
    :param config: (dict) a top level configuration object containing the 'loss' key
    :return: an instance of the loss function
    ...
    losses = _creat_multi_loss(name, loss_config, weight, ignore_index, pos_weight)
...
def _creat_multi_loss(names, loss_config, weight, ignore_index, pos_weight):
    losses = []
    for name in names:
        loss = _create_loss(name,loss_config, weight, ignore_index, pos_weight)
        losses.append(loss)
