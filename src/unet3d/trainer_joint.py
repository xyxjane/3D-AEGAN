...
class UNet3DTrainer:
    ...
    def __init__(self, model, refine_model, optimizer_pre, lr_scheduler_pre,
                 optimizer_refine, lr_scheduler_refine,loss_criterion,
    ...
    def train(self):
        ...
        #                 f'Epoch [{self.num_epochs}/{self.max_num_epochs - 1}]')
            ...
            # final_output, pre_output, residual, estimated_res, loss, loss_refine = self._forward_pass(input, target, weight)
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                if not self.skip_train_validation:
                    ...
                    pre_eval_score = self.eval_criterion(preliminary_output, target)
                    train_eval_scores.update(eval_score.item(), self._batch_size(input))
                    train_pre_eval_scores.update(pre_eval_score.item(), self._batch_size(input))

                # log stats, params and images
                logger.info(
                    f'Training stats. Loss: {train_losses.avg}.  Residual Loss: {train_res_losses.avg}. Evaluation score: {train_eval_scores.avg}. Pre Evaluation score: {train_pre_eval_scores.avg}')
                self._log_stats('train', train_losses.avg, train_res_losses.avg, train_eval_scores.avg)
                self._log_params()
                self._log_images(input, target, final_output, pre_output, residual,estimated_res, 'train_')
    ...
    def validate(self):
        ...
        with torch.no_grad():
            ...
            # for i, t in enumerate(self.loaders['val']):
                ...
                pre_output, loss = self._forward_pass(self.model,input, target, weight)
                # final_output, pre_output, residual, estimated_res, loss, loss_pre = self._forward_pass(input, target, weight)
                ...
                if batch_idx % 100 == 0:
                    self._log_images(input, target, final_output, pre_output, residual, estimated_res, 'val_')
            ...
            self._log_stats('val', val_losses.avg, val_res_losses.avg, val_scores.avg)
    ...
    def _log_stats(self, phase, loss_avg, res_loss_avg, eval_score_avg):
        tag_value = {
