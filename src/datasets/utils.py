...
def get_patched_data(dataset_from_loader):
    patched_dataset = []
    for file_data in dataset_from_loader:
        for patched_data in file_data:
            patched_dataset.append(patched_data)
...
def get_train_loaders(config):
    ...
    train_transformer_config = loaders_config['train']['transformer']
    val_transformer_config = loaders_config['val']['transformer']
    #     # load slice builder config
    train_slice_builder_config = loaders_config['train']['slice_builder']
    val_slice_builder_config = loaders_config['val']['slice_builder']
    ...
    train_datasets = dataset_class(file_path=train_dataset_path, phase='train',slice_builder_config=train_slice_builder_config,\
    transformer_config=train_transformer_config)

    val_datasets = dataset_class(file_path = val_dataset_path, phase='val',slice_builder_config=val_slice_builder_config,\
    transformer_config=val_transformer_config)
    ...
    return {
        ...
        # don't shuffle during validation: useful when showing how predictions for a given batch get better over time
        'val': DataLoader(val_datasets, batch_size=batch_size, shuffle=True, num_workers=num_workers,pin_memory=True)
    }
