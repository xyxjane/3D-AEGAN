...
class HDF5Dataset(Dataset):
    def __init__(self, file_path,
        ...
        imagelist = glob.glob(self.file_path + "/*.h5")
        # train_drf_list=['drf10', 'drf20', 'drf4']
        train_drf_list = ['drf10','drf20', 'drf50', 'drf100']

        for image_path in imagelist:
            image_info = image_path.split('/')[-1]
            drf_info = image_info.split('_')[1]
            patient_num = image_info.split('_')[0]
            drf_num = image_info.split('_')[1]
            patient_number = int(patient_num)
            if patient_number >117:
                # if self.phase=='train':
                #     if drf_num in train_drf_list:
        ...
        print('Number of images in {}: {:d}'.format(self.phase, len(self.image_list)))
    ...
    def __getitem__(self, index):
        ...
        input_file = self.create_h5_file(self.image_list[index])
        self.raw = self.fetch_and_check(input_file, self.raw_internal_path)
        self.label = self.fetch_and_check(input_file, self.label_internal_path)
        if self.global_normalization:
            stats = calculate_stats(self.raw)
        else:
            stats = {'pmin': None, 'pmax': None, 'mean': None, 'std': None}

        self.transformer = transforms.Transformer(self.transformer_config, stats)
        self.raw_transform = self.transformer.raw_transform()
        self.label_transform = self.transformer.label_transform()

        raw_patch_transformed = self.raw_transform(self.raw)

        if self.phase == 'test':
            # discard the channel dimension in the slices: predictor requires only the spatial dimensions of the volume
            return raw_patch_transformed, index
        else:
            # get the slice for a given index 'idx'
            label_patch_transformed = self.label_transform(self.label)
            self._check_volume_sizes(raw_patch_transformed, label_patch_transformed)
            if self.weight_internal_path is not None:
                # look for the weight map in the raw file
                self.weight_map = self.fetch_and_check(input_file, self.weight_internal_path)
                self.weight_transform = self.transformer.weight_transform()
                weight_patch_transformed = self.weight_transform(self.weight_map)
...
class PatchedHDF5Dataset(Dataset):
    def __init__(self, patches_datasets):
        self.patches_datasets = patches_datasets
        
    def __getitem__(self, index):
        raw = self.patches_datasets[index][0]
        label = self.patches_datasets[index][1]
        # Unsqueeze channel dimension
        return (raw, label)
    
    def __len__(self):
        return len(self.patches_datasets)
...
class AbstractHDF5Dataset(ConfigDataset):
    ...
    def __init__(self, file_path,
        ...
        # self.weight_slices = slice_builder.weight_slicess
    ...
    def create_datasets(cls, dataset_config, phase):
        ...
        for file_path in file_paths:
            ...
            patient_number = int(patient_num)
            print(patient_drf)
            if patient_number > 117 and patient_drf=='drf100':
                try:
